...
void func_run(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref) {
  ...
  mmio_mem perf_mem(262144 * 4,
                    "../vivado/func_test_v0.01/soft/func/obj/main.bin");
  ...
  confreg.set_trace_file(
      "../vivado/func_test_v0.01/cpu132_gettrace/golden_trace.txt");
  ...
  while (!Verilated::gotFinish() && sim_time > 0 && running) {
    ...
    running = confreg.do_trace(top->debug_wb_pc, top->debug_wb_rf_wen,
                               top->debug_wb_rf_wnum, top->debug_wb_rf_wdata);
    if (top->debug_wb_pc == 0xbfc00100u)
      running = false;
    if (trace_on && func_start >= ticks) {
      vcd.dump(ticks);
      sim_time--;
    }
...
void perf_run(Vmycpu_top *top, axi4_ref<32, 32, 4> &mmio_ref,
  ...
  mmio_mem perf_mem(
      262144 * 4,
      "../vivado/perf_test_v0.01/soft/perf_func/obj/allbench/inst_data.bin");
  assert(mmio.add_dev(0x1fc00000, 0x100000, &perf_mem));
  ...
  for (int test = test_start; test <= test_end && running; test++) {
    ...
    while (!Verilated::gotFinish() && sim_time > 0 && running && !test_end) {
      ...
      if (top->aclk && top->aresetn)
        mmio_sigs.update_input(mmio_ref);
      if (top->aclk && top->aresetn && top->debug_wb_pc != last_pc &&
          top->debug_wb_pc)
        count++, last_pc = top->debug_wb_pc;
      top->eval();
      if (top->aclk && top->aresetn) {
        confreg.tick();
        mmio.beat(mmio_sigs_ref);
  ...
  if (test_end) {
    ...
    fout << "scores = " << std::fixed << std::setprecision(3)
         << std::pow(mulscores, 0.1) << "\n";
  }
  printf("=================IPC=====================\n");
      ...
      if (top->debug_wb_rf_wen && top->debug_wb_rf_wnum) {
        ...
        if (cemu_mips.debug_wb_pc != top->debug_wb_pc ||
            ...
            (cemu_mips.debug_wb_wdata != top->debug_wb_rf_wdata &&
             !cemu_mips.debug_wb_is_timer)) {
          printf("Error!\n");
          printf("reference: PC = 0x%08x, wb_rf_wnum = 0x%02x, wb_rf_wdata = "
                 "0x%08x\n",
                 cemu_mips.debug_wb_pc, cemu_mips.debug_wb_wnum,
                 cemu_mips.debug_wb_wdata);
          printf("mycpu    : PC = 0x%08x, wb_rf_wnum = 0x%02x, wb_rf_wdata = "
                 "0x%08x\n",
